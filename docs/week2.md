...
In looking for the presence or absence of trends in calculated viscosity as a function of sphere diameter, you will need to consider some estimate of uncertainties. Just as in Module 1, it is important to know how the difference in calculated viscosity for various sphere diameters compares to the random error.  You have a couple of choices as to how to deal with this.  (1) Your Week 1 analysis of 3.5-mm spheres can give you a sense of the random error at least for that one sphere diameter.  You can use this to help you estimate the random error in each viscosity.  Alternately, (2) you can follow a procedure similar to what you did in Module 1:  analyze multiple videos of each sphere diameter in order to make independent estimates of random error for each diameter.

Here is a subtle but important point that illustrates the difference between standard deviation and standard error of the mean.  If you use just a single trial for each sphere diameter other than 3.5 mm, how much would you expect those *single-trial* values of viscosity to differ from each other, just because of random error?  The answer to that is best captured by an uncertainty in viscosity calculated from the *standard deviation* of a set of repeated trials.  But if you use multiple trials for each diameter, how much would you expect all of the *average-based* values of viscosity to vary, just because of random error?  The answer to that is best captured by an uncertainty in viscosity calculated from the *standard error of the mean* values.

## Checkpoint 2

+ Submit your Week 2 results in [Checkpont 2 on Gradescope](https://www.gradescope.com/courses/165932/assignments/654616){:target="_blank"} by the end of Week 2.  You should submit (1) a scatter plot of calculated viscosity vs. sphere diameter, with no lines between points.  Your axes should be labeled with the quantity shown and the units used.  For sphere diameters where you have analyzed multiple videos, the vertical error bar should be based on *standard error of the mean* from your repeated trials.  For sphere diameters where you have analyzed just one video, the best available estimate of uncertainty is to find the uncertainty in viscosity based on the *standard deviation* of your repeated 3.5-mm trials, and approximate that the uncertainty in viscosity for other diameters will be similar to this.  You should also submit (2) a list of the sphere diameters for which you believe inertial drag is negligible.
