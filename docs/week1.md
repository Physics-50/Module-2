...
At your station you will have two diffraction gratings. One labelled as 500 lines/mm and the other 1000 lines/mm. This week we will only be working with the 500 line/mm set of gratings. This spacing is marked on the gratings - make sure you are using the 500 line/mm gratings and put aside the 1000 line/mm grating for now, you will not need it this week. Position the 500 line/mm diffraction grating between the laser and screen.
...
Let's focus on the angle of the laser beam, which should ideally come in perpendicular to the diffraction grating. What range of angles away from perfectly perpendicular to the laser beam might the grating accidentally have? Perhaps we might not notice if we were off by angles up to $$\pm 5^{\circ}$$. How important is this? To answer this question, set up your experiment with <em>L</em> = 60 cm. First, carefully set the diffraction grating perpendicular to the laser beam. You can do this by looking at the beam reflected by the diffraction grating. Adjust the diffraction grating so that the reflected beam retraces the incident beam (at least in the horizontal direction) by adjusting the diffraction grating until a spot from the reflected beam is visible on the laser itself. Speak to your instructor if unclear and **remember never to put your eye in the path of a laser beam**. To measure $$x$$, use a ruler or meter stick to measure the distance from the central bright spot to the next bright spot to one side. Now purposefully "wiggle" the grating by adjusting the angle of the grating to $$\approx 5^{\circ}$$ relative to the perpendicular. At this stage we are just trying to determine if the angle plays a significant role so it is okay to just estimate the angle. Now repeat the test with the grating rotated by $$\approx 5^{\circ}$$ relative to the perpendicular in the other direction.
 ...
 **Remember to reset all the other parameters that also have significant effects on $$x$$, for each measurement.** For example, you should reset the distance $$L$$ between each of the five trials for each grating.  
