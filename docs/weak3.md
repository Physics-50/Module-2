...
You can now directly read off the best fit value of viscosity $$\rho$$, with uncertainty, from the quoted fitting parameters. Remembering to follow the sig fig rules described in Module 1 (give link), in this example we would find that $$\rho = BLAH \pm bleh$$. 
